          ...
          <tr>
              <td style="vertical-align: middle;">
                  @if($travel_report->report_id == $trip_id)
                      ...
                      {{ get_report($travel_report->same_trip_id)->title }} @if(get_report($travel_report->report_id)->report_option == 'report')
                           <p>Travel Report with Travel Report</p>
                           @endif
                           {{-- @if(get_report($travel_report->report_id)->report_option == 'diary')
                           <p>Travel Report with Travel Diaries</p>
                           @endif --}}
                           @if(get_report($travel_report->report_id)->report_option == 'offer')
                           <p>Travel Report with Travel Proposal</p>
                           @endif
                  @else

                      {{ get_report($travel_report->report_id)->title }}
                      @if(get_report($travel_report->report_id)->report_option == 'report')
                           <p>Travel Report with Travel Report</p>
                           @endif
                           {{-- @if(get_report($travel_report->report_id)->report_option == 'diary')
                           <p>Travel Report with Travel Diaries</p>
