    ...
    <div class="row">
      <div class="col-md-3">
          <div class="badge-img">
              ...
              @else
                 <img src="{{url('img/frontend/purple.png')}}" class="img-responsive">
              @endif
          </div>
                ...
                <!-- <h4>@if($userdata){{isset($userdata->user->first_name)?$userdata->user->first_name:''}} {{isset($userdata->user->last_name)?$userdata->user->last_name:''}}@endif</h4> -->
                  ...
                  <div class="user_dt">
                    ...
                    {{-- @if(!empty($userdata->relation_status))
                       <p><b>Relationship Status:</b>@if($userdata) {{isset($userdata->relation_status)?$userdata->relation_status:''}}@endif</p>
                    @endif --}}
      ...
      <div class="col-md-9 right-detail m-auto">
           ...
           @if(!empty(Auth::user()))
            @if(Auth::user()->role_type=='travel_agency' || Auth::user()->role_type=='travel_blogger')
             @if(!empty($userdata))
              @if($user_id != Auth::user()->id)
                <div class="req-btn">
                  <a href="{{url('collaboration',encrypt_decrypt('encrypt',$user_id))}}"><button class="btn">COLLABORATION REQUEST</button></a>
                </div>
                @endif
              @endif
            @endif
          ...
          <div class="col-lg-4 col-md-6 col-sm-6">
                  ...
                  <div class="travel_over">
                   <b style="color: #005ca9;"><center>{{ $report_data->title}}</center></b>
                    @if(!empty($report_data->cover_photo) && file_exists(public_path('img/frontend/travel_report/coverphoto'.'/'.$report_data->cover_photo)) )
                      ...
                      @if(!empty(Auth::user()))
                          @if(Auth::user()->id==$user_id)
                            <div class="delete-report" style="float:right;"><a href="{{url('delete/travel_report',convertoToSlug($travel_report->title))}}" style="color: #ff00006e;"><i class="fas fa-trash"></i></a></div>

                            <div class="edit-report" style="float:right"><a href="{{url('edit/travel_report',convertoToSlug($travel_report->title))}}" style="color: #005ca9;"><i class="fas fa-edit"></i></a></div>
                          @endif
                      @endif
                        ...
                        <div class="middle">
                             @if(!empty($roledata->image) && file_exists(public_path('img/backend/traveler_image'.'/'.$roledata->image)) )
                               ...
                               @if($report_data->report_option=='report')
                                 ...
                                 <a href="{{url('view/travel_report',convertoToSlug($travel_report->title))}}" class="text"><img class="report-img" src="{{url('img/frontend/all_my_travel_reports_button.png')}}">Travel Report</a>
                                 @elseif($report_data->report_option=='diary')
                                 <a href="{{url('view/travel_report_dairy', convertoToSlug($report_data->title))}}" class="text"><img class="report-img" src="{{url('img/frontend/all_my_travel_reports_button.png')}}">Travel Diary</a>
                                 @endif
                          </div>
                        <!-- <div class="middle">
                ...
                <div class="travel_btn">
                    @if(!empty(Auth::user()))
                        ...
                        @endphp
                        @if($super=='1')
                            <a href="javascript:void(0)" class="travel_action status1 @if($userdata)@if($userdata->user_id==Auth::user()->id) disabled @endif @endif" data-value="super" data-id="{{$report_data->id}}" data-count="{{$report_data->supers_count}}" data-text="Like">Like {{$report_data->supers_count}}</a>
                        @else
                    ...
                    @if(!empty(Auth::user()))
                        ...
                        @endphp
                        @if($alert=='1')
                          <a href="javascript:void(0)" class="travel_action status1 @if($userdata)@if($userdata->user_id==Auth::user()->id) disabled @endif @endif" data-value="alert" data-id="{{$report_data->id}}" data-text="Alert" data-count="{{$report_data->alerts_count}}">Alert {{$report_data->alerts_count}}</a>
                        @else
