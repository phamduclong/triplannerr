    ...
    {{-- html()->textarea('page_description')
        ...
        ->attribute('maxlength', 191)
         ->attribute('value', $static_page_edit->description)
            ->required()
        ->required() --}}
<textarea class="form-control" name="page_description" placeholder="{{ __('validation.attributes.backend.access.static-page.page-description') }}" maxlength="191" required>
    @if(!empty($long_description))
        @foreach($long_description as $description)
            {{$description}}
        @endforeach
    @else
        {!! _($static_page_edit->description) !!}
    @endif
</textarea>
</div><!--col-->
</div><!--form-group-->
