            ...
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        {{ __('labels.backend.access.plan_month.management') }} <small class="text-muted">{{ __('labels.backend.access.plan_month.deactivated') }}</small>
                    </h4>
                </div><!--col-->
                            ...
                            <tr>
                                ...
                                <td><?php if($user->status==0){ ?>
                                  ...
                                  <a href="{{ route('admin.plan_month.status', $user) }}"><span class="badge badge-success" style="cursor:pointer">@lang('labels.general.active')</span></a>
                                      <?php } ?></td>
                                <td class="btn-td">@include('backend.plan_month.includes.actions', ['user' => $user])</td>
                            </tr>
                           <?php $i++ ?>
                        @endforeach
                        </tbody>
