...
@section('title', app_name() . ' | ' . __('labels.backend.access.services.edit-service'))
...
{{ html()->form('POST', route('admin.services.update', $service->id))->class('form-horizontal')->attribute('enctype', 'multipart/form-data')->open() }}
      ...
      <div class="col-sm-5">
        <h4 class="card-title mb-0">
          @lang('labels.backend.access.services.service')
            <small class="text-muted"> @lang('labels.backend.access.services.edit-service')
            </small>
        </h4>
      </div><!--col-->
    </div><!--row-->
      ...
      <div class="col">
        ...
        <div class="form-group row">
          {{ html()->label(__('validation.attributes.backend.access.service.page_description'))->class('col-md-2 form-control-label')->for('page_description') }}
            <div class="col-md-10">
              <textarea name="page_description" class="form-control">{!!@$service->description !!}
              </textarea>
            </div><!--col-->
        </div><!--form-group-->
