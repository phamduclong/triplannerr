...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data) : User
    {
        ...
        return DB::transaction(function () use ($data) {
            $role_type=($data['roles'])?$data['roles']:$data['roles'][0];
    ...
    public function update(User $user, array $data) : User
    {
        ...
        if (! isset($data['permissions']) || ! count($data['permissions'])) {
            ...
            {
                $image =$this->uploadImage($user,$data);
            }
        ...
        return DB::transaction(function () use ($user, $data,$image,$role_type) {
            if ($user->update([
                'first_name' => $data['first_name'],
                'last_name' => $data['last_name'],
                'user_name' => $data['user_name'],
                'email' => $data['email'],
                'avatar_location'=> $image,
                'role_type'=> $role_type,
               // 'role_type'=> ($data['roles'])?$data['roles']:$data['roles'][0],
            ])) {
                // Add selected roles/permissions
    ...
    public function uploadImage($user, array $data)
    {   
        if(!empty($user['avatar_location']))
        {
            if(file_exists(public_path('img/frontend/'.$user['avatar_location'])))
            {
                $image_path=public_path('img/frontend/'.$user['avatar_location']);
                unlink($image_path);
            }
        }
